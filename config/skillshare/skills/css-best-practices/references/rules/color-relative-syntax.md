...
```css
/* Baseline: explicit variant tokens */
:root {
  --brand: #6d5efc;
  --brand-lighter: #9287ff;
  --brand-darker: #4f42d0;
}

.lighter { color: var(--brand-lighter); }
.darker { color: var(--brand-darker); }

/* Upgrade when relative color syntax is supported */
@supports (color: oklch(from red l c h)) {
  .lighter { color: oklch(from var(--brand) calc(l + 0.2) c h); }
  .darker { color: oklch(from var(--brand) calc(l - 0.1) c h); }
}
```
